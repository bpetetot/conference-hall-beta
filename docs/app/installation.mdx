...
Create an Algolia app here: [Algolia app management](https://www.algolia.com/manage/applications)

Add to `.env.local` the Algolia **ID** and **Search-only Key**:
* `REACT_APP_ALGOLIA_ID=<ID>`
* `REACT_APP_ALGOLIA_SEARCH_KEY=<SEARCH_KEY>`

Add to cloud functions configuration the Algolia **ID** and **Admin Key**:

```
firebase functions:config:set algolia.id="<ID>" algolia.key="<ADMIN_KEY>"
```
...
* in webhooks list select `Delivered Messages` add your firebase cloud function `https://us-central1-FIREBASE_PROJECT.cloudfunctions.net/delivered`
